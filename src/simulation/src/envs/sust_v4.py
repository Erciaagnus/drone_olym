...
class SUST_v4(Env):
    class UAV:
        def __init__(self, state, v=17.0, battery = None):
            self.v = v
            self.dt = 0.05
            self.state = state
            ...
            self.charging = 0
        def copy(self):
            return SUST_v4.UAV(state=self.state.copy(), v=self.v, battery=self.battery)
        def move(self, action): # 여기서 action은 "yaw rate임" 즉 오메가(각속도)임... 그러니까 시발 계속 직진만 하지요
            dtheta = action*self.dt
            _lambda = dtheta / 2
            # state 업데이트.... self.v는 음... 하는 게 아니라 현재 속도값을 반영해야 할 거 같은데...
            if _lambda == 0.0:
                self.state[0] += self.v * self.dt * cos(self.state[-1]) # x 좌표 스테이트 마지막은 UAV 절대 theta(heading angle)
                self.state[1] += self.v * self.dt * sin(self.state[-1]) # y 좌표
            else:
    ...
    class Target:
        ...
        def copy(self):
            return SUST_v4(state = self.state.copy(), age=self.age, initial_beta = self.initial_beta, target_type = self.target_type, sigma_rayleigh = self.sigma_rayleigh)
        def cal_age(self):
    ...
    def __init__(self, r_max=80, r_min=0, dt=0.05, d=10.0, l=3, m=1, n=2, r_c =150, max_step = 6000, seed = None ):
        self.current_state = State()
        self.imu_data = Imu()
        self.action_controls = ActuatorControl()
        ...
        self.init_env() # mavros connecting
        ...
        self.seed = seed
        self.n_u = 2
        #Initialize Observation Space (Definition)
        obs_space = {}
    ...
    def state_cb(self, data):
        ...
        #print("State updated: ", self.current_state)
    # target_pose = ([[10,0], [10,3]], [0]*self.n) etc
    def lp_cb(self, msg):
        self.local_position = msg
        #print("Get Current Local Position")
    ...
    def reset(
        ...
        while self.current_state is None:
            print("Waiting for current_State to be updated")
            rospy.sleep(0.1)
        while self.local_position is None:
            print("Waiting for Subscribing current position")
            rospy.sleep(0.1)
        # uav_pose는 현재 uav_pose가 되어야지.
        if uav_pose is None:
            print("Initial Position is,,,", )
            uav_x = self.local_position_info.pose.pose.position.x
            uav_y = self.local_position_info.pose.pose.position.y
            uav_theta = 0
            uav_states = np.vstack([uav_x, uav_y, uav_theta]).T
            print("Initial Position is,,,(states)", uav_states)

        else:
            uav_states = uav_pose
        if batteries is None:
            batteries = np.random.randit(1500, 3000, self.m)
        else:
            batteries = batteries
        ...
        reset_pose = [0, 0, self.initial_altitude, 0]
        #print("Armed? ", self.current_state.armed)

        while True:
            if not self.current_state.armed:
                self.offb_arm()
            self.pose.pose.position.x = reset_pose[0]
            self.pose.pose.position.y = reset_pose[1]
            self.pose.pose.position.z = reset_pose[2]
            self.local_pos_pub.publish(self.pose)


            current_position_x = self.local_position.pose.position.x
            current_position_y = self.local_position.pose.position.y
            current_position_z = self.local_position.pose.position.z

            distance_to_goal = np.sqrt((reset_pose[0] - current_position_x)**2 +
                                       (reset_pose[1] - current_position_y)**2 +
                                       (reset_pose[2] - current_position_z)**2)

            if distance_to_goal < 0.1:  # 목표 위치에 충분히 가까워지면 루프를 종료합니다.
                print("Initial TAKE OFF SUCCESSFUL")
    ...
    def step(self, action):
        ...
        for uav_idx, uav_action in enumerate(action): # 인덱스 번호(UAV), Target 번호(uav_action)
            ...
            self.control_uav(uav_idx, uav_action) # uav_idx와 uav 타겟에 따라...-> uav 상태 업데이트....
            #self.yaw_rate = self.control_uav(uav_idx, uav_action) # uav_idx, uav_action 문제가 생김
            print('GO TO NEXT STEP (ORIENTATION).. WAITING 0.1 SECONDS')
            rospy.sleep(0.1)
            ...
            if action == 0: # Charge
                ...
                if (self.uavs[uav_idx].obs[0] < self.r_c):
                    if self.local_position.pose.position.z > 0.1:
                        self.pose.pose.position.x = self.local_position.pose.position.x
                        self.pose.pose.position.y = self.local_position.pose.position.y
                        self.pose.pose.position.z = 0
                        self.local_pos_pub.publish(self.pose)
                    else:
                        self.uavs[uav_idx].charging = 1 # charging 비율 선정하기
                        self.uavs[uav_idx].battery = min(self.uavs[uav_idx].battery + 10, 3000) # 일단 배터리 용량 3000mAh로 설정
                    # 착륙 후 일정 수준 이상 충전되면 이륙하기.
                        if self.uavs[uav_idx].battery >= 2000:
                            self.pose.pose.position.x = self.local_position.pose.position.x
                            self.pose.pose.position.y = self.local_position.pose.position.y
                            self.pose.pose.position.z = self.initial_altitude
                            self.local_pos_pub.publish(self.pose)
                else:  # not able to land on charge station(too far)
                    self.uavs[uav_idx].battery -= 1
                    w1_action = self.toc_get_action(self.uavs[uav_idx].obs[:2]) # Yaw Rate...
                    ...
                    # self.publish_velocity(self.uavs[uav_idx])
                    # self.maintian_altitude(self.uavs[uav_idx], self.initial_altitude)
    ...
    # def maintian_altitude(self, uav, target_altitude):
