...
class SUST_v3(Env):
    ...
    def __init__(self, render_mode: Optional[str]=None,
        ...
        self.setpoint_msg = mavros.setpoint.PoseStamped(header=mavros.setpoint.Header(frame_id="att_pose",stamp=None))
        ...
        current_file_path = os.path.dirname(os.path.abspath(__file__))
        self.distance_keeping_resutls = np.load(os.path.join(current_file_path, "80_dkc_result_0.0.npz"))
        self.distance_keeping_straightened_policy00 = self.distance_keeping_resutls["policy"]
        self.time_optimal_straightened_policy00 = np.load(os.path.join(current_file_path, "80_toc_result_0.0.npz"))
...
if __name__ == '__main__':
    rospy.init_node("make_envrionment")
    env=SUST_v3()
    rospy.sleep(1)
    obs = env.reset()
    done = False
