...
from mdp import States, Actions, Surveillanc_Actions, Rewards, StateTransitionProbability, Policy, MarkovDecisionProcess
...
class SUST_v2(Env):
    metada = {"render_modes": [ "human", "rgb_array"], "render_fps:":30}
    # Define UAV Agent
    '''
    ...
    '''
    class UAV:
        def __init__(self, state, v=1.0, battery=3000): # Need to others
            self.v =v
            self.dt = 0.05
            self.state=state
        ...
        # Copy Agent
        def copy(self):
            return SUST_v2.UAV(state=self.state.copy(), v=self.v, battery=self.battery)
        # MOVE
        def move(self, action):
            dtheta = action*self.dt # 그런데 action이 Heading Angle dlsrk
            _lambda = dtheta / 2
            if _lambda == 0.0: # Straight Line
                # state = [x, y, heading angle] -> heading angle과 속도 바탕으로 다음 x, y state 결정
                # Heading Angle은 학습된 결과
                self.state[0] += self.v*self.dt * cos(self.state[-1]) #  previous heading Angle
                ...
                self.state[2] += dtheta
                self.state[2] = wrap(self.state[2]) # Normalize the Angle
        @property
        def obs(self):
            x, y = self.state[:2]
            r = np.sqrt(x**2 + y**2)
    ...
    class Target:
        ...
        def copy(self):
            return SUST_v2.Target(state=self.state.copy(), age=self.age, initial_beta=self.initial_beta, target_type=self.target_type, sigma_rayleigh=self.sigma_rayleigh)
        def cal_age(self):
